...
import { freshnessColor, relativeTime, FRESH_HOURS, STALE_HOURS } from './freshness';
...
describe('freshnessColor', () => {
    it('green below the FRESH_HOURS threshold', () => {
        expect(freshnessColor(0)).toBe('#1d6b3a');
        expect(freshnessColor(FRESH_HOURS - 0.1)).toBe('#1d6b3a');
    });
    it('amber between FRESH_HOURS and STALE_HOURS', () => {
        expect(freshnessColor(FRESH_HOURS)).toBe('#7a6a3a');
        expect(freshnessColor(18)).toBe('#7a6a3a');
        expect(freshnessColor(STALE_HOURS - 0.1)).toBe('#7a6a3a');
    });
    it('red at and above STALE_HOURS', () => {
        expect(freshnessColor(STALE_HOURS)).toBe('#a32a2a');
        expect(freshnessColor(48)).toBe('#a32a2a');
    });
});

describe('relativeTime', () => {
    it('returns "just now" within the first hour', () => {
        expect(relativeTime(isoMinusHours(0.1), NOW)).toMatchObject({ label: 'just now' });
        expect(relativeTime(isoMinusHours(0.99), NOW)).toMatchObject({ label: 'just now' });
    });
    ...
    it('clamps future timestamps to "just now"', () => {
        ...
        expect(relativeTime(future, NOW)).toMatchObject({ label: 'just now', hoursAgo: 0 });
    });
});
