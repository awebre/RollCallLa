...
export function ProvenanceBadge({ source, term_source, style }: Props) {
  const badgeClass =
    "inline-flex items-center rounded-[3px] border px-[0.35rem] py-[0.05rem] text-[0.7rem]";

  if (source === "pdf") {
    return (
      <Tooltip
        content="Reconstructed from roll-call PDFs — no official chamber roster entry. Last name is all we have."
        className="ml-[0.4rem] align-middle"
      >
        <span
          className={`${badgeClass} font-semibold text-(--app-warn-text-badge) bg-(--app-warn-bg) border-(--app-warn-border)`}
          style={style}
        >
          PDF-only
        </span>
  ...
  if (term_source === "wikipedia" || term_source === "derived") {
    ...
    return (
      <Tooltip content={content} className="ml-[0.4rem] align-middle">
        <span
          className={`${badgeClass} font-medium text-(--app-badge-text) bg-(--app-badge-bg) border-(--app-badge-border)`}
          style={style}
        >
          {label}
        </span>
